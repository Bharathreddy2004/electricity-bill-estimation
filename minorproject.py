...
LIGHT_POWER_PER_HOUR = {"Incandescent": 60, "CFL": 15, "LED": 9}
...
REFRIGERATOR_POWER_PER_HOUR = 150
...
""",
  ...
  if state == "Select State":
    ...
    for day in days_of_week:
      ...
      st.sidebar.write("Note-Each step(0.25) represents 15 minutes")
      ...
      if state == "Andhra Pradesh":
        if total_power_consumption <= 100:
          ...
          bill_amount = 550 + (total_power_consumption - 200) * 4.5
        else:
          bill_amount = 1550 + (total_power_consumption - 400) * 6.0
        st.write(
        ...
        if total_power_consumption <= 50:
          bill_amount = total_power_consumption * 1.45
        elif total_power_consumption <= 100:
          bill_amount = 130 + (total_power_consumption - 50) * 2.60
        elif total_power_consumption <= 200:
          bill_amount = 335 + (total_power_consumption - 100) * 3.30
        elif total_power_consumption <= 400:
          bill_amount = 815 + (total_power_consumption - 200) * 4.30
        else:
